...
DELIMITER $$
CREATE TRIGGER pymt_whet_IS_TR AFTER INSERT ON dabeen.pymt
FOR EACH ROW
BEGIN
	DECLARE target_help_num CHAR(10);
	select h.help_num INTO target_help_num from pymt p INNER JOIN bskt b on p.pymt_num = b.bskt_num and p.pymt_num = new.pymt_num INNER JOIN bskt_comp bc on b.bskt_num = bc.bskt_num INNER JOIN help h on bc.help_num = h.help_num;
	UPDATE help SET pymt_whet = 'p' WHERE help_num = target_help_num;
END $$

-- CREATE TRIGGER pymt_whet_UP_TR (결제에 환불여부가 y로 업데이트 발생 시, 해당 결제에 해당하는 도움의 결제여부[pymt_whet]를 환불 상태('p')로 갱신하는 트리거)
DELIMITER $$
CREATE TRIGGER pymt_whet_UP_TR AFTER UPDATE ON dabeen.pymt
FOR EACH ROW
BEGIN
	DECLARE target_help_num CHAR(10);
    IF new.refd_whet = 'y'
    THEN
	select h.help_num INTO target_help_num from pymt p INNER JOIN bskt b on p.pymt_num = b.bskt_num and p.pymt_num = new.pymt_num INNER JOIN bskt_comp bc on b.bskt_num = bc.bskt_num INNER JOIN help h on bc.help_num = h.help_num;
    UPDATE help SET pymt_whet = 'r' WHERE help_num = target_help_num;
    END IF;
END $$
...
DELIMITER $$
CREATE TRIGGER pymt_end_help_IS_TR AFTER UPDATE ON dabeen.pymt
FOR EACH ROW
BEGIN
	DECLARE target_help_num CHAR(10);
    select h.help_num INTO target_help_num from pymt p INNER JOIN bskt b on p.pymt_num = b.bskt_num and p.pymt_num = new.pymt_num INNER JOIN bskt_comp bc on b.bskt_num = bc.bskt_num INNER JOIN help h on bc.help_num = h.help_num;
